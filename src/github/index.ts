  ...
  public static commits = async (options: any) => {
    ...
    try {
      ...
      if (!Array.isArray(data)) {
        console.log(`Github에서 데이터를 가져올 수 없습니다. 다시 시도해주세요.`);
        process.exit(0);
        return;
      }
      ...
      const additions = contributors.reduce((acc, item) => acc + item.additions, 0);
      const deletions = contributors.reduce((acc, item) => acc + item.deletions, 0);
