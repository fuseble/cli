...
export default class PrismaCLI {
  ...
  public static async getPrismaFiles(files: string[]) {
    ...
    const enumFile = rawResult.filter((item) => item.fileName === 'enum.prisma' || item.fileName === '_enum.prisma');

    if (Array.isArray(enumFile) && enumFile.length > 0) {
      enumFile.forEach((file) => {
        result += `${file.text}\n\n`;
      });
    }
    ...
    const relationFile = rawResult.filter(
      (item) => item.fileName === 'relation.prisma' || item.fileName === '_relation.prisma',
    );

    if (Array.isArray(relationFile) && relationFile.length > 0) {
      relationFile.forEach((file) => {
        result += `${file.text}\n\n`;
      });
    }

    console.log(`Prisma mixing done!`);
    return result;
  }
}
